 ...
 */
class TransformBalanceResourse extends Transform {
    ...
    _transform( chunk, encoding, callback ) {
        ...
        if( строка.length === 2 ) {
            ...
            итог.sum = строка[3] == undefined ? '' : строка[3].trim();
            итог['number_document'] = строка[4] == undefined ? '' : строка[4].trim();
            итог['date_document'] = строка[5] == undefined ? '' : строка[5].trim();
            итог['address_out_id'] = строка[6] == undefined ? '' : строка[6].trim();
            итог.period = this.period;
            callback(null,итог);
        }
    }
}
module.exports = TransformBalanceResourse;