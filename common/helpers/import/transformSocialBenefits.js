 ...
 */
class TransformSocialBenefits extends Transform {
    ...
    _transform( chunk, encoding, callback ) {
        ...
        if( строка.length === 2 ) {
            ...
            итог.code = строка[3] == undefined ? '' : строка[3].trim();
            итог.sum = строка[4] == undefined ? '' : строка[4].trim();
            итог.monetization = строка[5] == undefined ? '' : строка[5].trim();
            итог.method = строка[6] == undefined ? '' : строка[6].trim();
            итог.period = this.period;
            callback(null,итог);
        }
    }
}
module.exports = TransformSocialBenefits;